...
ブラウザで年間カレンダーを閲覧できる TypeScript + Vite 製の静的SPAです。デフォルトでは今年のカレンダーを表示し、任意の年に切り替えられます。`public/data/dummy_kabucom.csv` から証券会社（kabu.com）の取引データを読み込み、日別損益をカレンダーに重ねて可視化します。

## 主な機能

- **年間カレンダーの表示**: ブラウザ内で 12 ヶ月分のカレンダーデータを生成し、日曜始まりの表形式で描画します。
...
`http://localhost:5173` で年間カレンダーが表示されます。
...
`dist/` をそのまま GitHub Pages に配置すると静的配信できます。

## 別CSVファイルの指定

トップ画面の「CSVファイルの指定」セクションからローカルCSVを選択すると、ブラウザ内で解析して反映します。以降は同じファイルを保持したまま年を切り替えて再描画できます。
...
- 「エンドポイントをクリア」を押すと GAS URL/PSK の保存を解除し、既定の `public/data/dummy_kabucom.csv` に戻せます。
